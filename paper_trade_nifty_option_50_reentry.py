...
current_dt = IST.localize(datetime.combine(datetime.strptime(TRADE_DATE, "%Y-%m-%d"), TRADE_START))
...
while current_dt.time() <= TRADE_END:
    ...
    for candle, state, side in [
        ...
        # RETEST DETECTION AFTER TARGET
        if state["wait_for_retest"] and candle.close < state["marked"]:
            state["wait_for_retest"] = False
            print(f"🔄 {side} RETEST DONE — READY FOR NEW CYCLE")
        ...
        if (
            ...
            and not state["wait_for_retest"]     # 👈 ADD
    ...
    if combined_pnl >= 50:
        print("\n🎯 COMBINED TARGET HIT — RESETTING CYCLE\n")
        ...
        for state in (ce_state, pe_state):
            ...
            state["wait_for_retest"] = True    # 👈 WAIT MODE
